...
module.exports = class ReturnCall {
   ...
   draw(working, starty, mimic) {
      ...
      let lineColour =
         ...
         working.postdata.params && working.postdata.params.return &&
         Utilities.validColour(working.postdata.params.return.lineColour) ?
         working.postdata.params.return.lineColour :
         'rgb(0, 0, 0)';

      ///////////////////////
      // Get the arrow size
      let arrowSizeY = Utilities.isNumber(this._line.arrowSize) && this._line.arrowSize > 0 ?
      ...
      if (commentOnStartx) {
         let calltextxy = Utilities.drawTextRectangleNoBorderOrBg(ctx, textToPrint,
            returntmd, calltexty, startxAfterFlow + gapToText, null, null, mimic, wh);
         working.manageMaxWidth(calltextxy.x, calltextxy.y);
      } else {
         let calltextxy = Utilities.drawTextRectangleNoBorderOrBg(ctx, textToPrint,
            returntmd, calltexty, endxAfterFlow + (2 * arrowSizeY) + gapToText, null, null, mimic, wh);
         working.manageMaxWidth(calltextxy.x, calltextxy.y);
      }
